...
| AWS | Amplify app | `d22dm4b0jn71jw` / `wecare.digital` (app name and served host; apex + `www`, which 301s to it) | Frontend deployment and effective-header verification. Corrected 2026-09-26: this row named `stack.wecare.digital`, which is **NXDOMAIN** — the CNAME was removed 2026-09-25 and `GetApp` reports the app name as `wecare.digital`. Verify against `GetApp`/`ListDomainAssociations`, not against this row |
