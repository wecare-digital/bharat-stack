...
|---|---|---|---|
| **Tier 0 cleanup** (delete `[waId]`, `ai-config` tab, `contact-test`) | **None deleted — verified.** `ai-config-management` lambda is **dual-purpose**: besides the (disabled) WhatsApp auto-reply (`/ai/config`), it serves `/ai/internal/config` (the internal **FloatingAgent** assistant config — used by `dashboard/index.tsx` + `app/settings/internal-agent`) and `/ai/botflow` — both LIVE. `ai-generate-response` also powers FloatingAgent + subscribe/payment flows. So we only remove the `ai-config.tsx` notice PAGE/tab (zero API calls). Frontend-only. | none | very low |
