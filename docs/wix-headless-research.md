...
Auth today is an **API key** from Secrets Manager (`wecare/wix/headless-api-key`) plus
`WIX_SITE_ID` / `WIX_ACCOUNT_ID` — `handler.py:38-80`.

> **Current identifiers only.** Wix Account ID: `15f02319-40ff-4288-b8e6-69c791adae5e`; Headless Site ID:
> `fcd82f0c-9572-49c7-acfb-88fb05042ece`. Retired Editor site identifiers are intentionally omitted from current
> documentation. The single source of truth is
> [`src/config/wix.ts`](../src/config/wix.ts).
