...
| 1.2 | Confirm the target Wix site against the published/draft pair | ⏳ PENDING | agent | Now unblocked by 1.1. `https://xout.wecare.digital/` returned **404** during discovery, so the brief's "Published" claim does not hold at the URL it supplies; matching a Lambda env var to a committed constant proves self-consistency, not site identity. Settle it with `POST /site-list/v2/sites/query` |
