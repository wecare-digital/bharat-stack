...
| `SEC-ROUTE-001` | `/webhook/sinch-rcs` GET+POST is unauthenticated at gateway **and** handler; can forge messages and trigger outbound RCS sends | Phase 0 finding | `AuthorizationType=NONE`; `rcs-dlr/handler.py` has zero auth markers; invokes `wecare-rcs-send` from request body | Sinch signature verification, fail closed | `DISCOVERED` | — | `EV-0007` | Needs owner decision on live probe | Add failing test, implement signature check |
| `SEC-ROUTE-002` | `POST /whatsapp/inbound` is unauthenticated; consumes legacy SNS envelope; `action=create_invoice` runs before any auth | Phase 0 finding | `AuthorizationType=NONE`; no signature, no `require_auth` | Remove the public route or verify raw-body signature; make invoice path internal-only | `DISCOVERED` | — | `EV-0008` | Needs owner decision on live probe | Add failing test, then close the surface |
| `SEC-ROUTE-003` | `scripts/audit_route_auth.py` scans only one API and misses the `inbound-whatsapp-handler` directory mapping | Phase 0 finding | 3 routes unscanned; 1 false negative | Audit both APIs; resolve dir↔function names; gate in CI | `DISCOVERED` | — | `EV-0007` | — | Extend the script, re-baseline |
| `SEC-ROUTE-004` | Explicit authentication strategy for all 332 user API routes | overrides | 0 authorizers; 302 handler-authenticated, 2 OPEN, 4 unresolved on `zllr9lrg7j`; 3 unscanned on `79g3bbufdh` | Documented per-route decision + negative tests | `DISCOVERED` | — | `EV-0007` | — | Classify each route: signed webhook / handler auth / delete |
...
| `PROV-AIRTEL-001` | Zero Airtel messaging/voice executable surface; `rxairtel` VPA preserved | brief | No Airtel Lambda; 4 secrets scheduled; **3 dangling `/voice-in/cdr` routes remain** | Routes and integrations removed | `DISCOVERED` | `DEPLOYED` (runtime) | `EV-0007`, `EV-0009` | A4 approval to delete routes | Queue route deletion with backup evidence |
| `PROV-SINCHSMS-001` | Zero Sinch SMS surface; Sinch **RCS** preserved | brief | `wecare/sinch/sms` scheduled; **2 dangling `/webhook/sinch-dlr` routes remain** on `79g3bbufdh` | Routes and integrations removed; RCS untouched | `DISCOVERED` | `DEPLOYED` (runtime) | `EV-0009` | A4 approval to delete routes | Queue route deletion; never touch `wecare/sinch/rcs` |
