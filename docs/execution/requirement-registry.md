...
| `SEC-ROUTE-007` | `voice-in/obd` still contains an executable Airtel CDR-callback branch (`_is_cdr_callback` → `_handle_cdr_callback`) | 2026-09-21 finding | Now behind `require_auth`, so no longer anonymously reachable | Branch removed entirely | `DISCOVERED` | — | `EV-0021` | — | Fold into the Airtel retirement sweep |
| `SEC-ROUTE-008` | Destructive routes authenticate any signed-in user rather than requiring `Admin` | 2026-09-21 | `DELETE /voice-in/obd/clear-logs`, `DELETE /voice-in/c2c/clear-logs`, `POST /media/cleanup`, `DELETE /links/{code}` accept any valid Cognito token | `require_auth(event, 'Admin')` after confirming group membership | `DISCOVERED` | — | `EV-0021` | Must not lock out the current operator | Read Cognito group membership first, then tighten |
...
| `NOTIF-STORE-001` | `PstnNotificationDelivery` declared in four places, deployed nowhere | Phase 0 finding, resolved 2026-09-21 | Confirmed declared in `backend.ts`, `data/resource.ts`, `pstn/claims.py`, `pstn/keys.py`; absent from 66 tables. Inert only because `PSTN_CONNECTED_NOTIFICATIONS_ENABLED` is false | Phase 3 builds fresh notification stores and deletes this declaration rather than materialising it | `DISCOVERED` | — | `EV-0030`, `docs/execution/data-store-inventory.md` | Phase 3 design | Do **not** create the table as-is |
...
| `NOTIF-STORE-003` | Declared Amplify models and deployed tables are two disjoint naming worlds | 2026-09-21 finding | 65 models vs 66 tables, almost no name correspondence (`Contact` vs `ContactsTable`). `backend.ts` TTL override is wrapped in try/catch that logs and continues, so a mismatch there is silent | Establish whether the models are deployed at all | `DISCOVERED` | — | `docs/execution/data-store-inventory.md` | — | Phase 1: do not treat `resource.ts` as a description of production until settled |
| `NOTIF-STORE-004` | Six table names referenced by code do not exist in the account | 2026-09-21 finding | `SmsAwsTable` (3 files, legacy delete path only), `AirtelC2CTable`, `AirtelSMSTable`, `PstnNotificationDelivery`, `SystemConfig` (wrong name for `SystemConfigTable`), `wix-store` | Each removed, corrected, or accepted with a reason | `DISCOVERED` | — | `EV-0030` | — | `scripts/audit_data_model_drift.py --gate` once cleared |
| `PROV-AIRTEL-002` | Deployed env residue: `wecare-voice-in-c2c` carries `AIRTEL_C2C_TABLE` and `AIRTEL_C2C_SECRET_NAME=wecare/airtel/c2c` | 2026-09-21 finding | Table does not exist; secret is scheduled for deletion | Both removed from function configuration | `DISCOVERED` | — | `EV-0030` | — | Remove with the Airtel source sweep so code and config change together |
...
| `DEPLOY-003` | 3 functions serve routes with **no `live` alias**, so `update-function-code` reaches production instantly with no alias rollback | Phase 1 finding | `wecare-partner-onboarding` (21 routes), `wecare-marketing-ads` (2), `wecare-seo-tools` (2) — 25 unqualified integrations in total | Adopt the publish-and-move-alias model, or record why not | `DISCOVERED` | — | `EV-0032` | — | Create aliases, repoint integrations, then verify |
| `OPS-001` | 9 Lambda log groups have no retention set | Phase 1 finding | `ad-attribution`, `catalog-management`, `meta-analytics`, `plivo-answer`, `push-notifications`, `seo-tools`, `service-api`, `site-language`, `sla-engine` | A retention policy consistent with the data class | `DISCOVERED` | — | `EV-0032` | — | Unbounded cost and unbounded retention of log contents |
| `OPS-002` | 95 route paths are mentioned by no frontend file | Phase 1 finding | Mix of provider webhooks, internal-only routes and possibly dead surface | Each classified: webhook, internal, or retire | `DISCOVERED` | — | `EV-0032` | — | Join to callers before any deletion |
