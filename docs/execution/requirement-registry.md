...
The four rows still `DISCOVERED` were each **verified open**, not left stale:

| Row | Verified on 2026-09-25 |
|---|---|
| `SEC-ROUTE-007` | The Airtel CDR branch is still present at `voice-in/obd/handler.py:142-143,745,768` and `voice-in/c2c/handler.py:159`. Auth-gated, so not anonymously reachable |
| `SEC-ROUTE-008` | Zero `Admin` checks in the three destructive handlers. Blocked behind Admin group membership, which nobody holds |
| `PROV-AIRTEL-002` | 4 Airtel env keys still live on the two `voice-in` functions |
| `OPS-002` | Deliberately **not** re-measured: 361 routes are live now, so the earlier 95 is stale and re-deriving it is the next action |
...
| `SEC-ROUTE-007` | `voice-in/obd` still contains an executable Airtel CDR-callback branch (`_is_cdr_callback` → `_handle_cdr_callback`) | 2026-09-21 finding | Branch **still present** 2026-09-25: `voice-in/obd/handler.py:142-143,745,768` and `voice-in/c2c/handler.py:159`. Behind `require_auth`, so not anonymously reachable. Both functions are live: 40 and 39 invocations in 30d across 16 routes | Branch removed entirely | `DISCOVERED` | — | `EV-0041` (2026-09-25 re-measurement) | — | Scoped code change then deploy BOTH live functions. Not a delete-the-function job: they serve the OBD campaign and C2C UI |
...
| `PROV-AIRTEL-002` | Deployed env residue: `wecare-voice-in-c2c` carries `AIRTEL_C2C_TABLE` and `AIRTEL_C2C_SECRET_NAME=wecare/airtel/c2c` | 2026-09-21 finding | Confirmed live 2026-09-25. `wecare-voice-in-c2c` carries `AIRTEL_C2C_SECRET_NAME`, `AIRTEL_C2C_TABLE`, `AIRTEL_KONG_HOST`; `wecare-voice-in-obd` carries `AIRTEL_OBD_SECRET_NAME` | Both removed from function configuration | `DISCOVERED` | — | `EV-0041` (2026-09-25 re-measurement) | — | Remove the 4 keys with the CDR-branch change so code and config move together, in one deploy of each function |
...
| `OPS-002` | 95 route paths are mentioned by no frontend file | Phase 1 finding | Not re-measured 2026-09-25; 361 routes now live, so the earlier 95 is stale | Each classified: webhook, internal, or retire | `DISCOVERED` | — | `EV-0041` (2026-09-25 re-measurement) | — | Re-derive against 361 routes, join to callers, then decide per route |
