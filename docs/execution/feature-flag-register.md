...
| `WA_QA_RECIPIENT` | WhatsApp | unset (`shared/lambda_utils/live_smoke.py:RECIPIENT_ENV`) | **absent** in production, measured 2026-09-21 | dev / QA only | **RESOLVED 2026-09-22** — owner nominated `+918100640044`; see `.kiro/steering/02-qa-recipient.md`. Verified: not a business number, India/DLT-eligible, accepted by the lockdown while customers and business numbers are refused | — | suffix only, never the number. Note its last4 `0044` matches WABA2, so disambiguate on channel/direction | — | with the flag above | implemented, value known, **still unset in production** |
