...
- Primary WhatsApp identity: +919330994400, phone ID 1016149501586345, WABA 2094615664435155. Preserve its ownership, registration, approved templates, history and relationships. Verify secondary identities from actual account inventory.
...
|---|---|---|---|
| PayU gateway | Local commit `468744bb` removes `PayUWebhookLog`, its TTL registration and dead cleanup registry entry after confirming the table had already been deleted with zero items. Other scripts/UI/docs must still be scanned | No PayU Lambda, API route or table was found in the refreshed runtime inventory. Secret `wecare/payu` is scheduled for deletion and remains recoverable during its AWS recovery window | Treat the model cleanup as `CODE_COMPLETE` only until the commit is pushed, tested on its exact SHA and relevant IaC/deployment readback proves no recreation. Remove any remaining residue, retain deletion evidence and verify permanent secret absence after the recovery window |
...
| `MCP` | MCP discovery, governed tools and dashboard agent actions |
...
|---|---|---|
| Git state | During this audit `30aa8dcb` was pushed, then Kiro created local commit `468744bbfd81a09cf2b80f4a9a1458e05b088d82` (`feat(security): audit route auth; fix deploy retries; retire the PayU model`). At the final Plivo prompt update it was one commit ahead of `origin/stack`; several Kiro/audit files remained untracked | Treat this as volatile concurrent work. Re-read status/local/remote SHAs before each phase, never assume `468744bb` is pushed or deployed without readback, never reset to a snapshot SHA and never stage unrelated artifacts |
...
| GitHub | Exact SHA `30aa8dcb` completed Provider Policy run `35486162069` and CodeQL run `35486162192` successfully. Local successor `468744bb` had not been pushed at the last readback and therefore had no remote CI/deployment proof. No open PR was observed; branch was unprotected with no required check/review gate | Green CodeQL means scan execution, not zero vulnerabilities. Test/push the exact intended successor through the normal gate, protect `stack` through an approved repository-setting change, require relevant checks/review, and retain run URLs/evidence |
...
| Payments | Razorpay webhook/invoice/payment pages and generic Payment entities exist | PayU model/live table/UI references remain; exposed Razorpay live pair is a manual owner action; invoice agent action is placeholder | Razorpay only; remove PayU everywhere after evidence gate; reconcile Meta/Razorpay IDs and statuses; never give AI raw capture/refund authority | payments Lambdas/models/pages/secrets/webhooks | 0, 4, 9 | consumer inventory, HMAC/idempotency/reconciliation/test-mode evidence, zero PayU inventory; credential-dependent live checks pending owner update |
...
| Payments | invoice engine, payment reads, Razorpay webhook; Payment/Order/invoice/Razorpay log | Razorpay/Meta payment state and generic accounting records | PayU model/table/UI and placeholder AI invoice success | Deterministic payment command/query API; payment writes excluded from initial MCP release |
...
## Part F — WECARE Operations MCP and governed dashboard AI agent

Build one internal operations plane and multiple thin clients. Do **not** expose provider credentials, arbitrary HTTP, arbitrary SIP destinations, raw Lambda invocation or the existing Bedrock action-group handler. Do **not** make production traffic depend on Kiro, ChatGPT or Claude. Vendor MCPs remain developer/control-plane aids; the WECARE Operations MCP is the business-facing, tenant-aware interface.
...
```text
ChatGPT custom app     Claude connector       Kiro IDE       WECARE dashboard agent
       |                     |                   |                      |
       +---------------------+-------------------+----------------------+
                                     |
         ...
         redacted READ tools                  PLAN -> APPROVE -> APPLY
                 |                                        |
                 +-------------------+--------------------+
                                     v
             ...
             +-----------------------+-------------------------+
             |                       |                         |
      Meta Direct adapter       Plivo Voice adapter    AWS SMS / allowed RCS
      template/voice note       call/voice-message     channel adapters
             |                       |                         |
             +-----------------------+-------------------------+
                                     v
                   provider acceptance + verified receipt/callback
                                     |
                     append-only audit + Common Inbox status
```
...
### Dashboard AI Agent & MCP admin experience

Build an inner admin experience reachable from WhatsApp Business > Assistant & Automation and Platform Operations, without adding a fourth communications entry. It must show:
...
- natural-language task composer whose default is preview/PLAN; the separate approval screen displays exact redacted recipient, rendered content/action, channel/provider, compliance and cost before APPLY;
...
- tutorial tabs with current/target backend and frontend text diagrams, authentication steps for each client, safe examples, failure modes, official links and troubleshooting.

The dashboard agent and remote MCP clients must call the same versioned query/command service. They must produce the same plan hash and policy decision for identical actor/tenant/input. No frontend component may directly call Meta, Plivo, Razorpay, Google People, DynamoDB or a provider secret endpoint.

### AI/MCP security and quality gates

Test cross-tenant access, insufficient role/scope, expired/revoked token, audience/issuer mismatch, tool-schema drift, prompt injection in contact/message/tool output, Unicode/E.164 ambiguity, duplicate apply, changed plan, replayed approval, high-volume fanout, cost limit, DNC/calling hours, blocked country, unapproved template, unsafe media URL, malware/oversize media, provider timeout-after-accept, callback replay/out-of-order, model hallucinated tool/parameter and kill-switch behavior.
...
Create `docs/wecare-operations-mcp.md`, `docs/agent-tool-policy.md`, `docs/agent-client-compatibility.md` and a generated, secret-free tool manifest. Include cost controls for Bedrock/model tokens, AgentCore/gateway, Lambda, provider calls/messages and media/transcription. Budgets and alarms are guardrails; they do not authorize spend.
...
| Wix | Production site ID `c17b0e20-d96d-4fa1-b05c-bc97c04b4ac5`; account ID `15f02319-40ff-4288-b8e6-69c791adae5e` observed in repository | Protected existing site/account | `ListWixSites`/site context safe read, current Velo revision/deployment, domains, permissions and commerce app state; never create a replacement site |
...
### Dashboard AI feature scope

The dashboard agent may answer evidence-backed questions, compare periods, explain metric definitions, identify anomalies, draft a campaign/catalog/profile change plan, prepare a Plivo call/voice-message or an approved WhatsApp template reply, and report final provider states. It may call only the versioned WECARE Operations tools. It may not accept arbitrary provider URLs/Graph paths/GAQL, reveal credentials, execute raw Lambda/provider calls, invent contacts, send to an unverified number, change ad spend/status, publish Play releases, edit GBP, create a Wix site, upload Wix media or mutate catalog without the exact tool policy and approval.

For communication commands launched from the dashboard, retain all existing rules: resolve Contact and consent; show exact recipient, WABA/Plivo number, approved template/message/media, estimated provider cost and idempotency key; distinguish WhatsApp voice note from Plivo voice-message phone call; require approval; reconcile WAMID/CallUUID and final receipt. The growth integrations never become alternate SMS/WhatsApp/voice providers.
...
**Phase 6 — governed operations service, remote MCP and dashboard agent**
...
- Define and test the versioned READ/PLAN/APPLY tool catalog, immutable plan hashes, approvals, idempotency, jobs, receipts, audit and kill switches. Deploy read-only/status tools first; then PLAN tools. Keep every APPLY tool disabled until the exact staging/client/security gate passes.
...
| Governed AI/operations MCP | One tenant-aware operations service shared by dashboard, Bedrock and remote MCP; versioned READ/PLAN/APPLY tools; sends/calls require matching plan, explicit approval, idempotency and final provider reconciliation | Live agent/alias readiness, generated tool manifest, policy/evaluation suite, audit trail, kill-switch tests and Kiro/ChatGPT/Claude/dashboard compatibility evidence |
...
| Governed dashboard agent and WECARE Operations MCP |  |  |  |  |  |  |  |  |  |  |
| WhatsApp messaging |  |  |  |  |  |  |  |  |  |  |
| WhatsApp Calling |  |  |  |  |  |  |  |  |  |  |
| Plivo PSTN/browser |  |  |  |  |  |  |  |  |  |  |
