...
const CodeRepo: React.FC<PageProps> = ( { signOut, user } ) => {
  ...
  // AI auto-response toggle — PERMANENTLY DISABLED
  ...
  const loadConfig = useCallback( async () => {
    ...
    try
    {
      ...
      // AI config loading removed — feature permanently disabled
    } catch
    {
      // Use defaults
    } finally
  ...
  return (
    <Layout user={ user } onSignOut={ signOut }>
      ...
      <div className="inner-page" style={ { padding: '20px 24px', maxWidth: 900 } }>
        ...
        { activeTab === 'welcome' && (
          <div>
            {/* AI Auto-Response — REMOVED */ }
            <div style={ { ...S.card, borderLeft: '3px solid #9ca3af', opacity: 0.6 } }>
              <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } }>
                <div>
                  <label style={ { fontSize: 14, fontWeight: 600, color: '#6b7280' } }>AI Response Smoothing — Removed</label>
                  <p style={ { margin: '2px 0 0', fontSize: 11, color: '#9ca3af' } }>
                    WhatsApp AI auto-reply has been permanently disabled. Messages are handled by keyword triggers only.
                  </p>
                </div>
              </div>
            </div>
