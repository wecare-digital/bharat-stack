...
const SEOPageDetail: React.FC<PageProps> = ( { signOut, user } ) => {
  ...
  async function handleInspect () {
    ...
    try
    {
      ...
      toast.success( 'Inspection queued' );
  ...
  async function handleRegenSchema () {
    ...
    try
    {
      ...
      setSchema( data );
    } catch ( e: any ) { toast.error( e.message || 'Schema regeneration failed' ); }
  }
        ...
        <div style={ { display: 'flex', gap: 2, borderBottom: '2px solid #e5e7eb', marginBottom: 16 } }>
          { tabs.map( t => (
            <button key={ t } onClick={ () => setTab( t ) }
              style={ {
                padding: '10px 20px', fontSize: 14, fontWeight: tab === t ? 600 : 400, background: 'none', border: 'none',
                    ...
                    { inspections.map( ( insp: any ) => (
                      <tr key={ insp.id } style={ { borderBottom: '1px solid #f3f4f6' } }>
                        <td style={ { padding: '8px 12px' } }>
                          <span style={ {
                            background: insp.index_status === 'indexed' ? '#dcfce7' : '#fef2f2',
                            color: insp.index_status === 'indexed' ? '#166534' : '#dc2626',
                            padding: '2px 8px', borderRadius: 6, fontSize: 12
                          } }>{ insp.index_status }</span>
                        </td>
