      ...
      <div className="inner-page-container" style={{ background: '#fff' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {!editMode ? (
              <button onClick={() => setShowUnlock(true)} style={{ padding: '8px 16px', background: '#f9fafb', border: '1.5px solid #d1f470', borderRadius: 13, fontSize: 13, cursor: 'pointer', color: '#6b7280' }}>
                🔒 Read Only
              </button>
            ) : (
        ...
        {/* Unlock Modal */}
        {showUnlock && (
          <div role="dialog" aria-modal="true" aria-label="Unlock Edit Mode" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowUnlock(false)}>
            <div style={{ background: '#fff', borderRadius: 14, padding: 24, width: 360, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
              <h3 style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 600, color: '#374151' }}>Unlock Edit Mode</h3>
              <p style={{ fontSize: 13, color: '#6b7280', margin: '0 0 16px' }}>Enter admin password to enable configuration changes.</p>
              <input type="password" value={unlockPassword} onChange={e => setUnlockPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleUnlock()} placeholder="Password" style={{ width: '100%', padding: '10px 14px', border: '2px solid #f3f4f6', borderRadius: 13, fontSize: 14, outline: 'none', marginBottom: 12, boxSizing: 'border-box' }} autoFocus />
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                <button onClick={() => setShowUnlock(false)} style={{ padding: '8px 16px', background: '#fff', border: '2px solid #f3f4f6', borderRadius: 13, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
                <button onClick={handleUnlock} style={{ padding: '8px 20px', background: '#d1f470', color: '#1a3a2a', border: 'none', borderRadius: 13, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Unlock</button>
              </div>
            </div>
          </div>
        )}
                          ...
                          {Object.entries(fn.envVars).map(([key, val]) => (
                            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                              ...
                              {editMode ? (
                                ...
                                <code style={{ fontSize: 12, color: '#6b7280', wordBreak: 'break-all' }}>{val.includes('***') ? '••••••••' : val}</code>
                              )}
                            </div>
                          ))}
                        </div>
