...
const EmailCampaignPage: React.FC<PageProps> = ({ signOut, user }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setContacts(contactsData.filter(c => c.email));
      const messages = await api.listMessages(undefined, 'EMAIL');
      ...
      campaignMsgs.forEach(m => {
        ...
        if (!campaignMap.has(cid)) { campaignMap.set(cid, { id: cid, name: (m as any).campaignName || cid, subject: (m as any).subject || '-', recipients: 0, sent: 0, delivered: 0, opened: 0, failed: 0, status: 'completed', createdAt: m.timestamp }); }
        const c = campaignMap.get(cid)!; c.recipients++;
        if (m.status === 'sent' || m.status === 'delivered') c.sent++;
        if (m.status === 'delivered') c.delivered++;
        if (m.status === 'opened') c.opened++;
        if (m.status === 'failed' || m.status === 'bounced') c.failed++;
      });
  ...
  const handleSendCampaign = async () => {
    ...
    if (!subject.trim()) { setMessage({ type: 'error', text: 'Subject required' }); return; }
    if (!emailContent.trim()) { setMessage({ type: 'error', text: 'Email content required' }); return; }
    ...
    try {
      const campaignId = 'EC' + Date.now();
      let sent = 0, failed = 0;
      for (const contactId of selectedContacts) {
        const contact = contacts.find(c => c.contactId === contactId);
        if (!contact?.email) { failed++; continue; }
        try {
          const result = await api.sendEmailMessage(contactId, subject, emailContent);
      ...
      setCampaignName(''); setSubject(''); setEmailContent(''); setSelectedContacts([]); setSelectAll(false); loadData();
      ...
      <div className="inner-page campaign-page">
        ...
        {activeTab === 'create' && (
          <div className="form-section">
            <div className="form-group"><label>Campaign Name *</label><input type="text" value={campaignName} onChange={e => setCampaignName(e.target.value)} placeholder="My Email Campaign" /></div>
            <div className="form-group"><label>Subject *</label><input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="Email subject line..." /></div>
            <div className="form-group"><label>Email Content *</label><RichTextEditor value={emailContent} onChange={setEmailContent} placeholder="Write your email content..." channel="email" /></div>
            <div className="contacts-section">
              ...
              <div className="contacts-grid">
                {filteredContacts.slice(0, 100).map(c => (<label key={c.contactId} className={'contact-item' + (selectedContacts.includes(c.contactId) ? ' selected' : '')}><input type="checkbox" checked={selectedContacts.includes(c.contactId)} onChange={() => toggleContact(c.contactId)} /><span className="contact-name">{c.name || c.email}</span><span className="contact-phone">{c.email}</span></label>))}
                {filteredContacts.length === 0 && <div className="empty-state">No contacts with email found</div>}
              </div>
            </div>
            <Button variant="primary" className="btn-block" onClick={handleSendCampaign} disabled={sending || selectedContacts.length === 0 || !emailContent.trim() || !subject.trim()} loading={sending}>{sending ? 'Sending...' : `Send Campaign to ${selectedContacts.length} Contacts`}</Button>
          </div>
        )}
        {activeTab === 'logs' && (
          <div className="table-container"><table><thead><tr><th>Campaign</th><th>Subject</th><th>Recipients</th><th>Sent</th><th>Delivered</th><th>Opened</th><th>Failed</th><th>Date</th></tr></thead><tbody>
            {campaigns.map(c => (<tr key={c.id}><td><strong>{c.name}</strong></td><td>{c.subject}</td><td>{c.recipients}</td><td className="text-success">{c.sent}</td><td>{c.delivered}</td><td>{c.opened}</td><td className="text-error">{c.failed}</td><td>{new Date(c.createdAt).toLocaleDateString()}</td></tr>))}
            {campaigns.length === 0 && <tr><td colSpan={8} className="empty-state">No campaigns yet</td></tr>}
          </tbody></table></div>
        )}
      </div>
    </Layout>
