    ...
    <>
      <div className={`whatsapp-inbox ${mobileShowChat ? 'mobile-chat-active' : ''}`}>
        ...
        <div className="contacts-sidebar">
          ...
          <div className="contacts-list">
            ...
            {!loading && filteredContacts.length === 0 && <div style={{ padding: '40px 20px', textAlign: 'center', color: '#6b7280' }}>No contacts with email</div>}
          </div>
        </div>
          ...
          {selectedContact ? (
            <>
              ...
              <div className="messages-area">
                  ...
                  return (
                    <React.Fragment key={msg.id}>
                      ...
                      <div className={`message-bubble ${msg.direction}`}>
                        {msg.subject && <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4, color: '#374151' }}>{msg.subject}</div>}
                ...
                {filteredMessages.length === 0 && <div className="empty-chat"><div className="empty-chat-icon">📧</div><h3>No emails</h3><p>Emails with this contact will appear here</p></div>}
      ...
      <Modal isOpen={showCompose && !!selectedContact} onClose={() => setShowCompose(false)} title="Compose Email" size="lg" footer={<><Button variant="secondary" onClick={() => setShowCompose(false)}>Cancel</Button><Button variant="primary" onClick={handleSend} disabled={sending || !subject.trim() || !messageText.trim()} loading={sending}>{sending ? 'Sending...' : 'Send Email'}</Button></>}>
        {selectedContact && (<>
          ...
          <div className="form-group"><label>Subject</label><input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="Enter subject..." /></div>
