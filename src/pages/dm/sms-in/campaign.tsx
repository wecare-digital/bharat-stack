...
interface CampaignLog { id: string; name: string; recipients: number; received: number; replied: number; failed: number; status: string; createdAt: string; }
...
const SmsInCampaignPage: React.FC<PageProps> = ({ signOut, user }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      campaignMsgs.forEach(m => {
        ...
        if (!campaignMap.has(cid)) { campaignMap.set(cid, { id: cid, name: (m as any).campaignName || cid, recipients: 0, received: 0, replied: 0, failed: 0, status: 'completed', createdAt: m.timestamp }); }
        const c = campaignMap.get(cid)!; c.recipients++;
        if (m.status === 'received' || m.status === 'delivered') c.received++;
        if (m.status === 'replied') c.replied++;
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      const campaignId = 'SI' + Date.now();
      let sent = 0, failed = 0;
      for (const contactId of selectedContacts) {
        const contact = contacts.find(c => c.contactId === contactId);
        if (!contact?.phone) { failed++; continue; }
        try {
          const response = await fetch(`${API_BASE}/sms-aws/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contactId, content: messageContent, messageType: 'TRANSACTIONAL', campaignId, campaignName, channel: 'SMS_IN' }) });
  ...
  return (
    <Layout user={user} onSignOut={signOut}>
      <SEO title="SMS-IN Campaign | WECARE.DIGITAL" description="Send bulk SMS-IN campaigns" />
      <div className="inner-page campaign-page">
        <div className="page-header"><h2>SMS-IN Campaign</h2><Button variant="secondary" icon="refresh" iconOnly ariaLabel="Refresh" onClick={loadData} disabled={loading} loading={loading} /></div>
        ...
        {activeTab === 'create' && (
          <div className="form-section">
            <div className="form-group"><label>Campaign Name *</label><input type="text" value={campaignName} onChange={e => setCampaignName(e.target.value)} placeholder="My SMS-IN Campaign" /></div>
            <div className="form-group"><label>Message Content * ({messageContent.length}/160)</label><textarea value={messageContent} onChange={e => setMessageContent(e.target.value)} placeholder="Enter your SMS message..." rows={4} maxLength={160} /></div>
        ...
        {activeTab === 'logs' && (
          <div className="table-container"><table><thead><tr><th>Campaign</th><th>Recipients</th><th>Received</th><th>Replied</th><th>Failed</th><th>Date</th></tr></thead><tbody>
            {campaigns.map(c => (<tr key={c.id}><td><strong>{c.name}</strong></td><td>{c.recipients}</td><td className="text-success">{c.received}</td><td>{c.replied}</td><td className="text-error">{c.failed}</td><td>{new Date(c.createdAt).toLocaleDateString()}</td></tr>))}
