        ...
        <div style={{ padding: '12px 20px', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div><h1 style={{ fontSize: '18px', fontWeight: 600, margin: 0 }}>SMS IN Inbox</h1><span style={{ fontSize: '13px', color: '#6b7280' }}>Inbound Messages</span></div>
            </div>
              ...
              <>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {filteredMessages.map(msg => (
                    <div key={msg.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px', borderLeft: '3px solid #10B981' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ fontSize: '12px', color: '#6b7280' }}>↙ Received</span><span style={{ fontSize: '12px', color: '#9ca3af' }}>{new Date(msg.timestamp).toLocaleString()}</span></div>
                      <div style={{ fontSize: '14px', lineHeight: 1.5, color: '#374151' }}>{msg.content}</div>
                    </div>
                  ))}
                  {filteredMessages.length === 0 && <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>No inbound messages from this contact</div>}
                </div>
              </>
            ) : (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}><div style={{ fontSize: '48px', marginBottom: '16px' }}>📥</div><p style={{ margin: 0 }}>Select a contact</p><small>Choose a contact to view inbound messages</small></div>
            )}
          </div>
        </div>
      </div>
