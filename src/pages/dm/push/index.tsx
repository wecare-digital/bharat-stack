...
const PushPage: React.FC<PageProps> = ({ signOut, user, embedded }) => {
  ...
  const [apnsBundleId, setApnsBundleId] = useState('digital.wecare.stackcrm');
  const [configSaving, setConfigSaving] = useState(false);

  const loadDevices = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/push/devices`);
      if (res.ok) {
    ...
    try {
      ...
      if (pushTarget === 'user' && pushUserId) {
        ...
        await fetch(`${API_BASE}/push/send`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...payload, broadcast: true, platform: pushPlatform }),
        });
      }
                    ...
                    <tbody>
                      {devices.map((d, i) => (
                        <tr key={i}>
                          ...
                          <td style={{ fontFamily: 'monospace', fontSize: 11, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.deviceToken}</td>
          ...
          {activeTab === 'config' && (
            <div className="push-config">
              ...
              <div className="push-config-section">
                <div className="push-config-header">
                  ...
                  <div>
                    ...
                    <p>Configure FCM server key for Android push notifications</p>
                  </div>
                </div>
                <div className="push-config-fields">
                  <div className="push-field">
                    <label>FCM Server Key / Service Account JSON</label>
                    <textarea rows={4} value={fcmKey} onChange={e => setFcmKey(e.target.value)} placeholder="Paste your Firebase Server Key or service account JSON here" style={{ fontFamily: 'monospace', fontSize: 12 }} />
                  </div>
                </div>
              </div>
              ...
              <div className="push-config-section">
                <div className="push-config-header">
                  ...
                  <div>
                    ...
                    <p>Configure APNs authentication key for iOS push notifications</p>
                  </div>
                </div>
                <div className="push-config-fields">
                  <div className="push-config-grid">
                    <div className="push-field">
                      <label>Key ID</label>
                      <input type="text" value={apnsKeyId} onChange={e => setApnsKeyId(e.target.value)} placeholder="e.g. ABC123DEF4" />
                    </div>
                    <div className="push-field">
                      <label>Team ID</label>
                      <input type="text" value={apnsTeamId} onChange={e => setApnsTeamId(e.target.value)} placeholder="e.g. A1B2C3D4E5" />
                    </div>
                    <div className="push-field">
                      <label>Bundle ID</label>
                      <input type="text" value={apnsBundleId} onChange={e => setApnsBundleId(e.target.value)} placeholder="digital.wecare.stackcrm" />
                    </div>
                  </div>
                  <div className="push-field">
                    <label>APNs Auth Key (.p8)</label>
                    <textarea rows={4} value={apnsKey} onChange={e => setApnsKey(e.target.value)} placeholder="-----BEGIN PRIVATE KEY-----&#10;Paste .p8 key content&#10;-----END PRIVATE KEY-----" style={{ fontFamily: 'monospace', fontSize: 12 }} />
                  </div>
                </div>
              </div>
              ...
              <div className="push-info-box">
                <BellIcon size={18} />
                <div>
                  <strong>AWS SNS Platform Applications</strong>
                  <p>Push notifications are delivered via AWS SNS. Create two Platform Applications in the AWS Console:</p>
                  <ul>
                    <li>stack-wecare-push-android (FCM) for Android</li>
                    <li>stack-wecare-push-ios (APNs) for iOS</li>
                  </ul>
                  <p>Device tokens are stored in DynamoDB (PushTokensTable) and mapped to SNS endpoints.</p>
                </div>
              </div>

              <Button variant="primary" onClick={() => setConfigSaving(true)} disabled={configSaving} loading={configSaving} style={{ marginTop: 12 }}>
                Save Configuration
              </Button>
            </div>
          )}
