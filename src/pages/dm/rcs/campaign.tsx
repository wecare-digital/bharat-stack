...
const RcsCampaignPage: React.FC<PageProps> = ( { signOut, user, embedded } ) => {
  ...
  const handleSendCampaign = async () => {
    ...
    try
    {
      ...
      for ( const contactId of selectedContacts )
      {
        ...
        try
        {
          const response = await fetch( `${API_BASE}/rcs/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify( { phoneNumber: contact.phone, text: messageContent, metadata: JSON.stringify( { campaignId, campaignName, contactId } ) } ) } );
  ...
  const content = (
    <div className="inner-page campaign-page">
      <div className="page-header"><h2>RCS Campaign</h2><Button variant="secondary" icon="refresh" onClick={ loadData } disabled={ loading } loading={ loading }>Refresh</Button></div>
