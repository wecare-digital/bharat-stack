...
const VoiceCampaignPage: React.FC<PageProps> = ({ signOut, user }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const messages = await api.listMessages(undefined, 'VOICE');
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      ...
      for (const contactId of selectedContacts) {
        ...
        try {
          const response = await fetch(`${API_BASE}/voice-aws/call`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contactId, message: voiceMessage, campaignId, campaignName }) });
  ...
  return (
    <Layout user={user} onSignOut={signOut}>
      <SEO title="Voice Campaign | WECARE.DIGITAL" description="Send bulk voice call campaigns" />
      <div className="inner-page campaign-page">
        <div className="page-header"><h2>Voice Campaign</h2><Button variant="secondary" icon="refresh" iconOnly ariaLabel="Refresh" onClick={loadData} disabled={loading} loading={loading} /></div>
        ...
        {activeTab === 'create' && (
          <div className="form-section">
            <div className="form-group"><label>Campaign Name *</label><input type="text" value={campaignName} onChange={e => setCampaignName(e.target.value)} placeholder="My Voice Campaign" /></div>
