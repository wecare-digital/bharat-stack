/**
 * SMS Inbox Page - Black/White Theme
 */
...
const SmsInbox: React.FC<PageProps> = ({ signOut, user }) => {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const response = await fetch(`${API_BASE}/sms-aws/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contactId: selectedContact.id, content: messageText, messageType: 'TRANSACTIONAL' }) });
      if (response.ok) { toast.success('SMS sent successfully'); setMessageText(''); setShowCompose(false); await loadData(); } else { toast.error('Failed to send SMS'); }
      ...
      <div className="inbox-page">
        <div className="inbox-header">
          ...
          <div className="inbox-header-content">
            <div className="inbox-title"><span className="inbox-icon"></span><div><h1>SMS Inbox</h1><span className="inbox-subtitle">AWS Pinpoint/SNS</span></div></div>
            <div className="inbox-actions">
              <Button variant="primary" onClick={() => setShowCompose(true)} disabled={!selectedContact}>Compose</Button>
            ...
            {selectedContact ? (
              <>
                ...
                <div className="messages-list">
                  ...
                  {filteredMessages.length === 0 && <div className="empty-messages">No messages with this contact</div>}
                </div>
              </>
            ) : (<div className="no-selection"><div className="no-selection-icon"></div><p>Select a contact</p><small>Choose a contact from the list to view messages</small></div>)}
          </div>
        </div>
        <Modal isOpen={showCompose && !!selectedContact} onClose={() => setShowCompose(false)} title="Send SMS" size="md" footer={<><Button variant="secondary" onClick={() => setShowCompose(false)}>Cancel</Button><Button variant="primary" onClick={handleSend} disabled={sending || !messageText.trim()} loading={sending}>{sending ? 'Sending...' : 'Send SMS'}</Button></>}>
          {selectedContact && (<><div className="form-group"><label>To</label><input type="text" value={`${selectedContact.name} (${selectedContact.phone})`} disabled className="form-input disabled" /></div><div className="form-group"><label>Message</label><RichTextEditor value={messageText} onChange={setMessageText} placeholder="Write your SMS..." channel="sms" showAISuggestions={true} contactContext={selectedContact.name} /></div></>)}
        </Modal>
      </div>
      <style jsx>{`
        .inbox-page { height: calc(100vh - 60px); display: flex; flex-direction: column; background: #fafafa; }
        .inbox-header { padding: 12px 20px; background: #fff; border-bottom: 1px solid #e5e5e5; }
        .inbox-header-content { display: flex; align-items: center; justify-content: space-between; margin-top: 8px; }
        .inbox-title { display: flex; align-items: center; gap: 12px; }
        .inbox-icon { width: 40px; height: 40px; background: #000; color: #fff; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 20px; }
        .inbox-title h1 { font-size: 18px; font-weight: 600; margin: 0; }
        .inbox-subtitle { font-size: 13px; color: #6b7280; }
        ...
        .no-selection-icon { width: 64px; height: 64px; background: #f5f5f5; border-radius: 16px; display: flex; align-items: center; justify-content: center; font-size: 32px; margin-bottom: 16px; }
        ...
        .form-input { width: 100%; padding: 12px 14px; border: 1px solid #e5e5e5; border-radius: 8px; font-size: 14px; box-sizing: border-box; }
