/**
 * SMS Inbox Page
 * Inbox view for SMS messages via AWS Pinpoint/SNS
 */
...
const SmsInbox: React.FC<PageProps> = ({ signOut, user }) => {
  ...
  return (
    <Layout user={user} onSignOut={signOut}>
      <SEO title="SMS Inbox | WECARE.DIGITAL" description="SMS inbox via AWS Pinpoint/SNS" />
        ...
        <div style={{ padding: '12px 20px', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <h1 style={{ fontSize: '18px', fontWeight: 600, margin: 0 }}>SMS Inbox</h1>
                <span style={{ fontSize: '13px', color: '#6b7280' }}>AWS Pinpoint/SNS</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setShowCompose(true)} disabled={!selectedContact} style={{ padding: '10px 18px', background: selectedContact ? '#10B981' : '#e5e7eb', color: selectedContact ? '#fff' : '#9ca3af', border: 'none', borderRadius: '13px', fontWeight: 500, cursor: selectedContact ? 'pointer' : 'not-allowed' }}>Compose</button>
              <Button variant="secondary" icon="refresh" iconOnly ariaLabel="Refresh" onClick={loadData} disabled={loading} loading={loading} />
            </div>
          </div>
        </div>

        {error && <div style={{ background: '#fef2f2', color: '#dc2626', padding: '8px 16px', fontSize: '13px' }}>{error}</div>}
              ...
              {paginatedContacts.map(contact => (
                <div key={contact.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px 16px', cursor: 'pointer', borderBottom: '1px solid #f5f5f5', background: selectedContact?.id === contact.id ? '#ECFDF5' : selectedIds.has(contact.id) ? '#f0fdf4' : 'transparent' }} onClick={() => setSelectedContact(contact)}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '14px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '6px' }}>{contact.name}{contact.unread > 0 && <span style={{ background: '#10B981', color: '#fff', fontSize: '11px', padding: '2px 6px', borderRadius: '10px' }}>{contact.unread}</span>}</div>
                    <div style={{ fontSize: '12px', color: '#6b7280' }}>{contact.phone}</div>
                    <div style={{ fontSize: '12px', color: '#9ca3af', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: '4px' }}>{contact.lastMessage || 'No messages'}</div>
                  </div>
                </div>
              ))}
              {filteredContacts.length === 0 && <div style={{ padding: '40px 20px', textAlign: 'center', color: '#6b7280' }}>No contacts with phone</div>}
            </div>
          </div>
              ...
              <>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {filteredMessages.map(msg => (
                    <div key={msg.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px', borderLeft: `3px solid ${msg.direction === 'inbound' ? '#10B981' : '#6b7280'}` }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ fontSize: '12px', color: '#6b7280' }}>{msg.direction === 'inbound' ? '↙ Received' : '↗ Sent'}</span><span style={{ fontSize: '12px', color: '#9ca3af' }}>{new Date(msg.timestamp).toLocaleString()}</span></div>
                      <div style={{ fontSize: '14px', lineHeight: 1.5, color: '#374151' }}>{msg.content}</div>
                      <div style={{ marginTop: '12px' }}><span style={{ fontSize: '12px', padding: '4px 8px', borderRadius: '6px', background: msg.status === 'delivered' ? '#ECFDF5' : '#f5f5f5', color: msg.status === 'delivered' ? '#065f46' : '#6b7280' }}>{msg.status}</span></div>
                    </div>
                  ))}
                  {filteredMessages.length === 0 && <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>No messages with this contact</div>}
                </div>
              </>
            ) : (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}><div style={{ fontSize: '48px', marginBottom: '16px' }}>📱</div><p style={{ margin: 0 }}>Select a contact</p><small>Choose a contact from the list to view messages</small></div>
            )}
          </div>
        </div>

        {showCompose && selectedContact && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowCompose(false)}>
            <div style={{ background: '#fff', borderRadius: '16px', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid #e5e7eb' }}><h2 style={{ margin: 0, fontSize: '18px' }}>Send SMS</h2><button onClick={() => setShowCompose(false)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#6b7280' }}>×</button></div>
              <div style={{ padding: '20px', flex: 1, overflowY: 'auto' }}>
                <div style={{ marginBottom: '16px' }}><label style={{ display: 'block', fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>To</label><input type="text" value={`${selectedContact.name} (${selectedContact.phone})`} disabled style={{ width: '100%', padding: '12px 14px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', background: '#f9fafb', boxSizing: 'border-box' }} /></div>
                <div style={{ marginBottom: '16px' }}><label style={{ display: 'block', fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>Message</label><RichTextEditor value={messageText} onChange={setMessageText} placeholder="Write your SMS..." channel="sms" showAISuggestions={true} contactContext={selectedContact.name} /></div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 20px', borderTop: '1px solid #e5e7eb' }}><Button variant="secondary" onClick={() => setShowCompose(false)}>Cancel</Button><Button variant="primary" onClick={handleSend} disabled={sending || !messageText.trim()} loading={sending}>{sending ? 'Sending...' : 'Send SMS'}</Button></div>
            </div>
          </div>
        )}
      </div>
