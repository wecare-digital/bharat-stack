...
interface CampaignLog { id: string; name: string; recipients: number; sent: number; delivered: number; failed: number; status: string; createdAt: string; }
    ...
    try {
      ...
      campaignMsgs.forEach(m => {
        ...
        if (!campaignMap.has(cid)) {
          campaignMap.set(cid, { id: cid, name: (m as any).campaignName || cid, recipients: 0, sent: 0, delivered: 0, failed: 0, status: 'completed', createdAt: m.timestamp });
        }
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      ...
      for (const contactId of selectedContacts) {
        ...
        try {
          const response = await fetch(`${API_BASE}/sms-aws/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contactId, content: messageContent, messageType: 'PROMOTIONAL', campaignId, campaignName }) });
  ...
  return (
    <Layout user={user} onSignOut={signOut}>
      ...
      <div className="inner-page campaign-page">
        <div className="page-header"><h2>SMS Campaign</h2><Button variant="secondary" icon="refresh" iconOnly ariaLabel="Refresh" onClick={loadData} disabled={loading} loading={loading} /></div>
        ...
        {activeTab === 'create' && (
          <div className="form-section">
            <div className="form-group"><label>Campaign Name *</label><input type="text" value={campaignName} onChange={e => setCampaignName(e.target.value)} placeholder="My SMS Campaign" /></div>
        ...
        {activeTab === 'logs' && (
          <div className="table-container"><table><thead><tr><th>Campaign</th><th>Recipients</th><th>Sent</th><th>Delivered</th><th>Failed</th><th>Date</th></tr></thead><tbody>
            {campaigns.map(c => (<tr key={c.id}><td><strong>{c.name}</strong></td><td>{c.recipients}</td><td className="text-success">{c.sent}</td><td>{c.delivered}</td><td className="text-error">{c.failed}</td><td>{new Date(c.createdAt).toLocaleDateString()}</td></tr>))}
