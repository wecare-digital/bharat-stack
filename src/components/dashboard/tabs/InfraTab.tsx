...
const LAMBDAS: LambdaInfo[] = [
  ...
  { name: 'wecare-outbound-sms', category: 'Messaging', purpose: 'Send SMS via Pinpoint/Airtel', apiRoute: '/sms/send', tables: ['ContactsTable', 'MessagesTable'], triggers: ['API Gateway'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
  ...
  { name: 'wecare-sms-aws', category: 'Messaging', purpose: 'AWS Pinpoint SMS handler', apiRoute: '/sms-aws', tables: ['ContactsTable', 'SmsAwsTable'], triggers: ['API Gateway', 'SNS'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
  { name: 'wecare-sms-in-airtel', category: 'Messaging', purpose: 'Airtel inbound SMS + DLT templates', apiRoute: '/webhook/sms-in', tables: ['AirtelSMSTable', 'DLTTemplates', 'ContactsTable'], triggers: ['API Gateway (Webhook)'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
  ...
  { name: 'wecare-voice-in-c2c', category: 'Messaging', purpose: 'Airtel Click-to-Call webhook', apiRoute: '/webhook/voice-c2c', tables: ['AirtelC2CTable', 'VoiceCDRTable'], triggers: ['API Gateway (Webhook)'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
  { name: 'wecare-voice-in-obd', category: 'Messaging', purpose: 'Airtel OBD campaign webhook', apiRoute: '/webhook/voice-obd', tables: ['OBDCampaignTable', 'VoiceCDRTable'], triggers: ['API Gateway (Webhook)'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
  ...
  { name: 'wecare-payu-webhook', category: 'Payments', purpose: 'PayU payment webhook handler', apiRoute: '/webhook/payu', tables: ['PaymentsTable', 'InvoicesTable', 'PayUWebhookLogTable'], triggers: ['API Gateway (Webhook)'], runtime: 'Python 3.12', timeout: 30, memory: 128, envVars: {} },
...
const TABLES: TableInfo[] = [
  ...
  { name: 'AirtelSMSTable', displayName: 'Airtel SMS', category: 'SMS', partitionKey: 'messageId', gsis: ['contactId-index', 'phoneNumber-index', 'status-index'], ttlField: 'expiresAt', usedBy: ['wecare-sms-in-airtel'] },
  { name: 'DLTTemplates', displayName: 'DLT Templates', category: 'SMS', partitionKey: 'templateId', gsis: [], usedBy: ['wecare-sms-in-airtel'] },
  ...
  { name: 'AirtelC2CTable', displayName: 'Airtel C2C', category: 'Voice', partitionKey: 'callId', gsis: ['contactId-index', 'fromNumber-index', 'toNumber-index', 'status-index'], ttlField: 'expiresAt', usedBy: ['wecare-voice-in-c2c'] },
  ...
  { name: 'PaymentsTable', displayName: 'Payments', category: 'Payments', partitionKey: 'id', gsis: ['paymentId-index', 'orderId-index'], usedBy: ['wecare-razorpay-webhook', 'wecare-payu-webhook', 'wecare-payments-read', 'wecare-invoice-engine'] },
  { name: 'InvoicesTable', displayName: 'Invoices', category: 'Payments', partitionKey: 'id', gsis: [], usedBy: ['wecare-invoice-engine', 'wecare-razorpay-webhook', 'wecare-payu-webhook'] },
  ...
  { name: 'PayUWebhookLogTable', displayName: 'PayU Webhook Log', category: 'Payments', partitionKey: 'id', gsis: [], ttlField: 'expiresAt', usedBy: ['wecare-payu-webhook'] },
...
const SECRETS: SecretInfo[] = [
  ...
  { name: 'wecare/airtel/c2c', purpose: 'Airtel Click-to-Call API credentials', keys: ['api_key', 'app_id'], usedBy: ['wecare-voice-in-c2c', 'wecare-outbound-voice'] },
  { name: 'wecare/airtel/sms', purpose: 'Airtel SMS API credentials', keys: ['username', 'password', 'customer_id'], usedBy: ['wecare-sms-in-airtel'] },
  { name: 'wecare/airtel/obd', purpose: 'Airtel OBD campaign credentials', keys: ['api_key'], usedBy: ['wecare-voice-in-obd'] },
];