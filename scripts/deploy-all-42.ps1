# Deploy ALL 42 wecare-* Lambda functions with updated handler.py code
...
$success = 0; $skip = 0; $fail = 0; $i = 0

foreach ($entry in $functions.GetEnumerator()) {
    $i++
    $fn = $entry.Key; $hp = $entry.Value
    Write-Host "[$i/$total] $fn..." -NoNewline
    if (-not (Test-Path $hp)) { Write-Host " SKIP" -ForegroundColor Yellow; $skip++; continue }
    $zp = "scripts\_tmp.zip"
    if (Test-Path $zp) { Remove-Item $zp -Force }
    Compress-Archive -Path $hp -DestinationPath $zp -Force
    $r = aws lambda update-function-code --function-name $fn --zip-file "fileb://$zp" --region $region --output text --query "FunctionName" 2>&1
    if ($LASTEXITCODE -eq 0) { Write-Host " OK" -ForegroundColor Green; $success++ }
    else { Write-Host " FAIL" -ForegroundColor Red; $fail++ }
    Remove-Item $zp -Force -ErrorAction SilentlyContinue
}

Write-Host "`n=== DEPLOY COMPLETE: $success OK / $skip SKIP / $fail FAIL (of $total) ==="
