...
def deploy_lambda(zip_bytes: bytes) -> str:
    ...
    env = {
        ...
        "SITE_LANGUAGE_ALLOWED_ORIGINS": "https://www.wecare.digital,https://wecare.digital",
...
def ensure_api_routes(function_arn: str) -> None:
    ...
    # Limit public spend at the API layer as a second guard behind request size caps.
    stages = api.get_stages(ApiId=API_ID, MaxResults="100").get("Items", [])
    for stage in stages:
        if stage.get("StageName") == "prod":
            api.update_stage(
                ApiId=API_ID,
                StageName="prod",
                DefaultRouteSettings={
                    **(stage.get("DefaultRouteSettings") or {}),
                    "ThrottlingBurstLimit": 30,
                    "ThrottlingRateLimit": 15.0,
                },
            )
            print("[api] ensured prod throttling (15 rps / burst 30)")
            break
