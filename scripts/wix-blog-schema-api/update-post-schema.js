...
const { WIX_API_KEY, WIX_SITE_ID, BRAND_NAME, BRAND_LOGO_URL, BASE_URL } = process.env;
...
function buildBlogPostingSchema(post) {
  return {
    ...
    url: post.url,
    author: { "@type": "Person", name: post.authorName || "Swdhya Vaksetu", url: post.authorUrl || `${BASE_URL}/swdhya` },
...
async function findPostBySlug(slug) {
  const r = await fetch(`https://www.wixapis.com/blog/v3/posts?paging.limit=100&fieldsToInclude=SEO`, { headers });
  if (!r.ok) return null;
...
async function updatePost(post) {
  ...
  console.log(`\n📝 Updating: "${post.title}" (slug: ${post.slug})`);
  ...
  if (!existing) { console.error(`  ❌ Not found: ${post.slug}`); return; }
  ...
  const r = await fetch(`https://www.wixapis.com/blog/v3/draft-posts/${existing.id}`, { method: "PATCH", headers, body: JSON.stringify(body) });
  const data = await r.json();
  if (!r.ok) { console.error(`  ❌ Update failed (${r.status}):`, JSON.stringify(data, null, 2)); return; }

  const jsonLdCount = (data.draftPost?.seoData?.tags || []).filter(t => t.type === "script").length;
  console.log(`  ✅ seoData updated`);
  console.log(`  📊 Structured data: ${jsonLdCount} JSON-LD schema(s) added`);

  // Publish
  const pub = await fetch(`https://www.wixapis.com/blog/v3/draft-posts/${existing.id}/publish`, { method: "POST", headers });
  console.log(`  🚀 Publish: ${pub.ok ? "✅" : "❌"} (${pub.status})`);
}

// ── Main ──
const inputFile = process.argv[2] || resolve(__dirname, "posts.json");
const posts = JSON.parse(await fs.readFile(inputFile, "utf8"));
console.log(`📦 ${posts.length} post(s) loaded`);
for (const post of posts) await updatePost(post);
console.log("\n✅ Done.");
