...
def _handle_flow_data(body: Dict, request_id: str, origin: str = '') -> Dict:
    ...
    if action == 'ping':
        ...
        NO_ORDER_FLOWS = ('subscribe', 'rx_slot', 'flows')  # 'flows' = Flows Builder preview

        if any(_ft_prefix.startswith(p) for p in NO_ORDER_FLOWS) or not flow_token:
            # Subscribe, Leave Feedback, Medical Visit, Flows Builder preview, or unknown
            # Navigate to the first screen of the flow (PERSONAL_INFO for subscribe)
            response_payload = {
                'version': decrypted_data.get('version', '4.0'),
    ...
    # enterprise_assist, schedule_appointment, leave_review, order_notes, subscribe
    if not response_payload and flow_token:
        ...
        if _is_generic:
            # Route by screen — intermediate screens pass data forward, final screen saves
            if screen == 'WELCOME':
                # WELCOME → FORM: pass any initial data to the form screen
                response_payload = {
                    'screen': 'FORM',
                    'data': data,
                }
                ...
                try:
                    ...
                    if data.get('request_id'):
                        item['orderId'] = data['request_id']
                    if data.get('name'):
                        item['senderName'] = data['name']
                    if data.get('subject'):
                        item['subject'] = data['subject']
                    if data.get('description') or data.get('notes') or data.get('review'):
                        item['description'] = data.get('description') or data.get('notes') or data.get('review')

                    fs_table.put_item(Item=item)
                    logger.info(json.dumps({
                ...
                # Any other screen — pass data forward to next screen
                response_payload = {
                    'screen': 'SUCCESS',
                    'data': data,
                }
    ...
    logger.info(json.dumps({
        'event': 'flow_response', 'action': action, 'screen': response_payload.get('screen', ''),
