...
def handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET':
            ...
            # GET /templates/{templateId} — single template details
...
def _extract_path_param(path: str, prefix: str) -> str:
    """Extract path parameter after a prefix, e.g. /templates/abc123 → abc123."""
...
def _list_templates(waba_id, query_params):
    try:
        kwargs = {'id': waba_id, 'maxResults': int(query_params.get('maxResults', 100))}
        response = social_messaging.list_whatsapp_message_templates(**kwargs)
        templates = []
        for t in response.get('templates', []):
            templates.append({
                'metaTemplateId': t.get('metaTemplateId'),
                'templateName': t.get('templateName'),
                'templateStatus': t.get('templateStatus'),
            })
...
def _list_template_library(waba_id, query_params):
    try:
        kwargs = {'id': waba_id, 'maxResults': int(query_params.get('maxResults', 50))}
        response = social_messaging.list_whatsapp_template_library(**kwargs)
        templates = []
        for t in response.get('metaLibraryTemplates', []):
            templates.append({
                'templateId': t.get('templateId'),
                'templateName': t.get('templateName'),
                'templateBody': t.get('templateBody'),
            })
        return {'statusCode': 200, 'headers': cors_headers(origin), 'body': json.dumps({'templates': templates})}
    except Exception as e:
        return _error_response(500, str(e))
